///
//  Generated code. Do not modify.
//  source: gender.proto
//
// @dart = 2.12
// ignore_for_file: annotate_overrides,camel_case_types,unnecessary_const,non_constant_identifier_names,library_prefixes,unused_import,unused_shown_name,return_of_invalid_type,unnecessary_this,prefer_final_fields

// ignore_for_file: UNDEFINED_SHOWN_NAME
import 'dart:core' as $core;
import 'package:protobuf/protobuf.dart' as $pb;

class Gender extends $pb.ProtobufEnum {
  static const Gender SOGenderMale = Gender._(0, const $core.bool.fromEnvironment('protobuf.omit_enum_names') ? '' : 'SOGenderMale');
  static const Gender SOGenderFemale = Gender._(1, const $core.bool.fromEnvironment('protobuf.omit_enum_names') ? '' : 'SOGenderFemale');

  static const $core.List<Gender> values = <Gender> [
    SOGenderMale,
    SOGenderFemale,
  ];

  static final $core.Map<$core.int, Gender> _byValue = $pb.ProtobufEnum.initByValue(values);
  static Gender? valueOf($core.int value) => _byValue[value];

  const Gender._($core.int v, $core.String n) : super(v, n);
}

